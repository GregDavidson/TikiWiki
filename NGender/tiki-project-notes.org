...
*** Keith doesn't have permissions!

**** Keith's Emacs

***** sql-buffer & sending code
sql-buffer is correctly set to *SQL* but
- the send commands are using the global value which is nil
- set-variable is not setting the global value
- sql-specific set command is not doing it
- had to use setq from *scratch*
- wtf???
